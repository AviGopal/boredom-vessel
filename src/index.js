...
const DISPATCHER_EXPLORATION_RATE = parseFloat(process.env.BOREDOM_DISPATCHER_EXPLORATION_RATE ?? "0.15");
...
async function writeBoredomSelectionSnapshot(input) {
    ...
    try {
        ...
        if (rhythmRes.ok) {
            const rhythmBody = await rhythmRes.json();
    ...
    try {
        const tick_at = new Date().toISOString();
        ...
        if (!res.ok)
            console.warn("[boredom] writeBoredomSelectionSnapshot: write failed", res.status, res.statusText);
    }
    catch (err) {
        console.warn("[boredom] writeBoredomSelectionSnapshot: publish failed", err);
    }
}
async function buildSkipSetFromLessons(failureClass) {
 ...
 */
async function handleDispatchGapFailure(gap_id, failure_class) {
    ...
    console.warn(`[boredom] gap-failure lesson updated: gap=${gap_id} class=${failure_class} skipped_count=${lesson?.skipped_count ?? 1}`);
}
const DISPATCHER_COMPARISON_INTERVAL = parseInt(process.env.BOREDOM_DISPATCHER_COMPARISON_INTERVAL ?? "50", 10);
const DISPATCHER_CYCLE_COUNTER_FILE = process.env.BOREDOM_DISPATCHER_COUNTER_FILE ?? "/tmp/boredom-dispatcher-cycle";
const GAP_FAILURE_LESSONS_FILE = process.env.BOREDOM_GAP_FAILURE_LESSONS_FILE ?? "/tmp/boredom-gap-failure-lessons.json";
async function recordGapFailureLesson(gap_id, failure_class) {
    let lessons = [];
    try {
...
async function getGapFailureLessons() {
    try {
        const raw = await Bun.file(GAP_FAILURE_LESSONS_FILE).text();
...
async function sampleLoad() {
    try {
        ...
        if (!body || typeof body !== "object")
            return null;
        const cpuStat = body["cpu_stat_cumulative"];
        const memBytes = body["mem_cgroup_current_bytes"];
        const load1m = body["load_avg_1m"];
        return {
            ...
            load_anomaly: body["load_anomaly"] === true,
            load_anomaly_severe: body["load_anomaly_severe"] === true,
        };
    }
    catch {
        return null;
...
async function writeLoadAttribution(record) {
    try {
...
async function recordLoadAttribution(ctx) {
    ...
    const cpu_usec_delta = haveBefore && haveAfter
        ? loadAfter.cpu_usec - ctx.load_before.cpu_usec
        : null;
    const mem_bytes_delta = haveBefore && haveAfter && ctx.load_before.mem_bytes !== null && loadAfter.mem_bytes !== null
        ? loadAfter.mem_bytes - ctx.load_before.mem_bytes
        : null;
    const load_1m_delta = haveBefore && haveAfter && ctx.load_before.load_1m !== null && loadAfter.load_1m !== null
    ...
    await writeLoadAttribution({
        ...
        sample_quality: haveBefore && haveAfter ? "both_present" :
...
function sampleExternalGoal(tick) {
    try {
...
const AUTONOMOUS_GOAL_TARGET_TEMPLATES = [
...
const SCENARIO_ROTATION_FALLBACK = [
...
const CONCEPT_BACKFILL_QUERIES = [
...
function extraVariablesForGoal(goalIdx) {
    if (goalIdx === 16) {
    ...
    if (goalIdx === 14) {
        ...
        return {
            ...
            manifest_body: `# Backend snapshot ${ts}\n\n` +
            ...
            pr_body: `Backend snapshot manifest authored by the substrate.\n\n` +
...
async function refreshExecutableResolvers() {
    try {
...
async function pickTopProposedGapClosingTemplate() {
    try {
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/templates?limit=60`, { headers: authHeaders() });
        if (!res.ok)
            return null;
        const data = await res.json();
        const candidates = (data.templates ?? []).filter(t => {
            ...
            if (!AUTHORED_PREFIXES.some(p => id.startsWith(p)))
                return false;
...
async function isIdle() {
    ...
    let res;
    try {
        res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/execution-traces?limit=20&since=${since}`, { headers: authHeaders() });
    }
    catch (err) {
        // If activity-api is unreachable, assume idle (fail-open for coverage).
        console.warn(`[boredom-vessel] activity-api unreachable, assuming idle: ${err.message}`);
        return true;
    }
    if (!res.ok) {
    ...
    try {
        ...
        traces = Array.isArray(body) ? body : (body.traces ?? []);
    }
    catch {
        return true;
    }
    ...
    if (externalTraces.length > 0) {
        console.log(`[boredom-vessel] ${externalTraces.length} external traces in last ${IDLE_WINDOW_SECONDS}s — skipping`);
        return false;
    }
    return true;
}
/** Read the persisted goal index without advancing — gating may move it. */
async function peekGoalIndex() {
    try {
...
async function advanceGoalIndex(dispatchedIdx) {
 ...
 */
function maxCostForLoad(sample) {
    if (sample === null)
        return "expensive";
    if (sample.load_anomaly_severe)
        return "cheap";
    if (sample.load_anomaly)
        return "moderate";
    return "expensive";
}
...
const RETIRED_GOAL_IDXS = new Set([16]);
function selectGoalForLoad(startIdx, maxCost) {
    ...
    for (let offset = 0; offset < n; offset++) {
        ...
        if (RETIRED_GOAL_IDXS.has(candidateIdx))
            continue;
        if (COST_RANK[cost] <= budgetRank)
            return candidateIdx;
    }
...
function gammaIntApprox(shape) {
    let acc = 0;
    const n = Math.max(1, Math.floor(shape));
    for (let i = 0; i < n; i++)
        acc += -Math.log(Math.max(1e-9, Math.random()));
    return acc;
}
 ...
 */
async function fetchCurrentSignature() {
    try {
        const res = await fetch(`${DEV_VESSEL_ENDPOINT}/v2/impulses/resolve`, {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `ApiKey ${API_KEY}` },
...
async function fetchPosteriorsForSignature(signature) {
    const cells = new Map();
    try {
        ...
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/execution-traces?limit=500&since=${since}`, { headers: authHeaders(), signal: AbortSignal.timeout(8_000) });
        if (!res.ok)
            return cells;
        const data = (await res.json());
        ...
        for (const t of arr) {
            ...
            const tid = (typeof t.activity_id === "string" ? t.activity_id : undefined)
            ...
            if (!cell) {
                cell = { alpha: 0, beta: 0, samples: 0 };
                cells.set(goalIdx, cell);
            }
            if (success)
                cell.alpha += 1;
            else
        ...
        try {
            ...
            const { readFileSync: _rf, existsSync: _ex } = require("node:fs");
            if (_ex(ledgerPath)) {
                const ledger = JSON.parse(_rf(ledgerPath, "utf8"));
                for (const [gIdx, cell] of cells) {
                    ...
                    for (const [det, entry] of Object.entries(ledger)) {
                        if (det && stem.includes(det)) {
                            closures += entry?.closures ?? 0;
                            matched = true;
                        }
                    }
                    if (!matched)
                        continue; // not a ledgered detector tick — leave alpha unchanged
...
async function selectGoalForLoadConditioned(startIdx, maxCost) {
    const rrPick = selectGoalForLoad(startIdx, maxCost);
    const signature = await fetchCurrentSignature();
    if (!signature)
        return { goalIdx: rrPick, mode: "round_robin", signature: null, cellsExamined: 0 };
    ...
    for (let offset = 0; offset < n; offset++) {
        ...
        if (RETIRED_GOAL_IDXS.has(idx))
            continue;
        if (COST_RANK[cost] <= budgetRank)
            eligible.push(idx);
    }
    if (eligible.length === 0)
        return { goalIdx: rrPick, mode: "round_robin", signature, cellsExamined: 0 };
    ...
    for (const idx of eligible) {
        ...
        if (cell && cell.samples >= MIN_CELL_SAMPLES) {
            anyConfident = true;
            break;
        }
    }
 ...
 */
async function tickAutoPromote() {
    try {
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/templates/auto-promote`, {
            method: "POST",
            headers: authHeaders(),
        ...
        console.log(`[boredom-vessel] auto-promote tick: considered=${body.considered ?? 0} promoted=${promoted.length}` +
            (promoted.length > 0 ? ` ids=[${promoted.map(p => p.template_id).join(",")}]` : ""));
    }
    catch (err) {
        console.warn(`[boredom-vessel] auto-promote tick error: ${err.message}`);
    }
}
...
async function tickExerciseProposal() {
    try {
        if (inFlight.size >= MAX_CONCURRENT)
            return; // pool saturated — yield to shape picks
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/templates/proposed-for-exercise?limit=40`, { headers: authHeaders(), signal: AbortSignal.timeout(5_000) });
        if (!res.ok) {
        ...
        if (allCandidates.length === 0) {
            console.log(`[exercise] no executable proposals (backlog=${body.backlog_total ?? 0}, ` +
        ...
        for (const e of inFlight.values())
            if (e.template_id)
                inFlightIds.add(e.template_id);
        let dispatched = 0;
        for (const t of candidates) {
            if (dispatched >= EXERCISE_BUDGET)
                break;
            if (inFlight.size >= MAX_CONCURRENT)
                break;
            const id = t.id;
            ...
            console.log(`[exercise] reserving proposal ${id} (class=${t.gap_class ?? "?"}, execs=${t.executions ?? 0}, ` +
            ...
            void (async () => {
                try {
                    ...
                    if (result) {
                        console.log(`[exercise] completed proposal ${id} outcome=${result.success ? "success" : "no_op"} ` +
                    ...
                    inFlight.delete(reserveId);
                    console.warn(`[exercise] dispatch error for ${id}: ${err.message}`);
                }
            })();
        }
    }
    catch (err) {
        console.warn(`[exercise] tick error: ${err.message}`);
    }
}
/**
 ...
 */
function deriveCapabilityHints(_goalIdx, targetTemplateId) {
    if (!targetTemplateId) {
    ...
    const isDeterministicChain = /-(tick|scan|audit|report|backfill)$/.test(targetTemplateId) ||
 ...
 * sampleBeta call).
 */
async function fetchDispatcherPosteriors(signature, goalIdx) {
    const cells = new Map();
    try {
        const since = Date.now() - 24 * 60 * 60 * 1000;
        ...
        for (const t of arr) {
            ...
            for (const tag of tags) {
                if (tag === "dispatcher_used:goal-host") {
                    dispatcher = "goal-host";
                    break;
                }
                if (tag === "dispatcher_used:light-dispatch") {
                    dispatcher = "light-dispatch";
                    break;
                }
            }
            if (!dispatcher)
 ...
 */
async function checkRecentOOMCount(_dispatcher) {
    return 0;
}
/**
...
async function selectDispatcher(goalIdx, signature, capability_hints) {
    // Hard filter: capability
    const eligible = ["goal-host"];
 ...
 */
async function dispatchGoal(dispatcher, goal, targetTemplateId, variables, tags, stateSignatureHash) {
    ...
    if (stateSignatureHash)
        requestBody.state_signature_hash = stateSignatureHash;
...
async function dispatchVesselAdditionScaffold() {
    try {
        ...
        console.warn(`[boredom-vessel] dispatchVesselAdditionScaffold error: ${err.message}`);
    }
}
async function main() {
    ...
    if (selection.mode === "thompson") {
        console.log(`[boredom-vessel] state-conditioned selection: signature=${selection.signature} ` +
            `mode=thompson cells_examined=${selection.cellsExamined} picked=${goalIdx} ` +
            `(round_robin would have picked ${roundRobinIdx})`);
    }
    else if (skipped > 0) {
        console.log(`[boredom-vessel] load-gating: maxCost=${maxCost} (load_anomaly=${loadBefore?.load_anomaly}, severe=${loadBefore?.load_anomaly_severe}) — ` +
    ...
    if (externalGoal)
        console.log(`[boredom-vessel] EXTERNAL-DEMAND slot: dispatching pool goal instead of rotation[${goalIdx}]`);
    ...
    const fireComparisonProbe = decision.eligible.length === 2 &&
        DISPATCHER_COMPARISON_INTERVAL > 0 &&
        cycle % DISPATCHER_COMPARISON_INTERVAL === 0;
    console.log(`[boredom-vessel] dispatcher selected: ${dispatcher} (reason=${decision.reason}, ` +
    ...
    if (fireComparisonProbe) {
        const probeDispatcher = dispatcher === "goal-host" ? "light-dispatch" : "goal-host";
        void dispatchGoal(probeDispatcher, goal, targetTemplateId, extraVars, [...baseTags, "comparison_probe:true", `comparison_probe_pair:${dispatcher}`], selection.signature)
            .catch((err) => {
            console.warn(`[boredom-vessel] comparison probe (${probeDispatcher}) failed: ${err.message}`);
        });
    }
    let res;
    try {
        ...
        console.error(`[boredom-vessel] ${dispatcher} dispatcher unreachable: ${err.message}`);
        ...
        if (dispatcher === "light-dispatch" && decision.eligible.includes("goal-host")) {
            ...
            dispatcher = "goal-host";
            try {
                res = await dispatchGoal("goal-host", goal, targetTemplateId, extraVars, [...baseTags, "dispatcher_fallback:from_light-dispatch"], selection.signature);
            }
            catch (err2) {
                console.error(`[boredom-vessel] fallback also failed: ${err2.message}`);
                process.exit(1);
            }
        }
        else {
    ...
    if (dispatcher === "light-dispatch") {
        console.log(`[boredom-vessel] dispatched via light-dispatch — executionId=${dispatch.executionId ?? "?"} status=${dispatch.status ?? "?"}`);
    ...
    if (!dispatch.dispatchId) {
        console.log(`[boredom-vessel] dispatched — executionId=${dispatch.executionId ?? "?"} status=${dispatch.status ?? "?"}`);
        await recordLoadAttribution({
    ...
    while (Date.now() < pollDeadline) {
        ...
        try {
            ...
            console.warn(`[boredom-vessel] poll error: ${err.message}`);
            continue;
        }
        if (!pollRes.ok)
            continue;
        ...
        if (poll.status === "completed" || poll.status === "failed") {
            let goalHostOutcome = "success";
            try {
                const execRes = await fetch(`${GOAL_HOST_ENDPOINT}/executions/${dispatchId}`, { headers: { Authorization: `ApiKey ${API_KEY}` }, signal: AbortSignal.timeout(10_000) });
                if (execRes.ok) {
            ...
            console.log(`[boredom-vessel] dispatched — executionId=${poll.executionId ?? "?"} status=${poll.status} reach_verdict=${goalHostOutcome}`);
            await recordLoadAttribution({
...
const HIGH_PRIORITY_GAP_CATEGORIES = new Set([
...
const MEDIUM_PRIORITY_GAP_CATEGORIES = new Set([
    "cost_constraint",
    "typecheck_error",
...
let priorityWeightByShape = new Map();
...
let learningModeEmphasis = null;
let modeWeightByMode = new Map();
...
function candidateMode(templateId, tags) {
    const hay = `${templateId} ${tags.join(" ")}`.toLowerCase();
    const has = (markers) => markers.some((m) => hay.includes(m));
    // reflect is the most specific family; check it first, then develop, then collect.
    if (has(MODE_REFLECT_MARKERS))
        return "reflect";
    if (has(MODE_DEVELOP_MARKERS))
        return "develop";
    if (has(MODE_COLLECT_MARKERS))
        return "collect";
    return null;
}
...
function isGapDrainCandidate(templateId, tags) {
    if (tags.some((t) => GAP_DRAIN_TAG_MARKERS.includes(t)))
        return true;
...
function gapPriorityWeight(g) {
    ...
    if ((g.id ?? "").toString().startsWith("baseline-typecheck-broken-"))
    ...
    if (sev === "medium" || hint === "medium")
        return PRIORITY_WEIGHT_MEDIUM;
    // otherwise derive from category
    const cat = (g.category ?? "").toString();
    if (HIGH_PRIORITY_GAP_CATEGORIES.has(cat))
        return PRIORITY_WEIGHT_HIGH;
    if (MEDIUM_PRIORITY_GAP_CATEGORIES.has(cat))
        return PRIORITY_WEIGHT_MEDIUM;
    return 1.0;
}
...
function priorityWeightForCandidate(templateId, outputShapes, tags) {
    let w = 1.0;
...
async function refreshSubstrateState() {
    ...
    try {
        const proposalDir = await fs.readdir("/workspace/proposals").catch(() => []);
    ...
    const nextPriorityByShape = new Map();
    let nextFloor = 1.0;
    try {
        ...
        if (gapsRaw) {
            ...
            const isOpen = (g) => g.status !== "closed" && g.status !== "resolved" && g.status !== "rejected";
            openGapCount = gaps.filter(isOpen).length;
            const scenarios = await fs.readdir(SCENARIOS_DIR).catch(() => []);
            const scenarioIds = new Set(scenarios.filter((n) => n.endsWith(".json")).map((n) => n.replace(/\.json$/, "")));
            unbridgedScenarioGapCount = gaps.filter((g) => g.status !== "closed" && (!g.scenario_id || !scenarioIds.has(g.scenario_id))).length;
    ...
    try {
        ...
        if (healthRaw) {
            ...
            if (sig === "red" || sig === "critical" || sig === "unhealthy") {
                if (PRIORITY_WEIGHT_HIGH > nextFloor)
                    nextFloor = PRIORITY_WEIGHT_HIGH;
            }
            else if (sig === "yellow" || sig === "degraded" || sig === "warning") {
                if (PRIORITY_WEIGHT_MEDIUM > nextFloor)
                    nextFloor = PRIORITY_WEIGHT_MEDIUM;
            }
        }
    }
    catch { /* ignore */ }
        ...
        if (lmRes.ok) {
            ...
            if (psb && typeof psb === "object") {
                for (const [shape, raw] of Object.entries(psb)) {
                    const wv = Number(raw);
                    if (!Number.isFinite(wv) || wv <= 1.0)
                        continue;
                    const cur = priorityWeightByShape.get(shape) ?? 1.0;
                    if (wv > cur)
                        priorityWeightByShape.set(shape, wv);
                }
            }
            // Cache mode weights.
            const nextModeWeights = new Map();
            const mw = body?.mode_weights;
            if (mw && typeof mw === "object") {
                for (const m of ["develop", "collect", "reflect"]) {
                    const wv = Number(mw[m]);
                    if (Number.isFinite(wv) && wv > 0)
                        nextModeWeights.set(m, wv);
                }
            }
            modeWeightByMode = nextModeWeights;
            if (learningModeEmphasis) {
                console.error(`[boredom-vessel] learningMode read: emphasize=${learningModeEmphasis} ` +
    ...
    try {
        ...
        if (rhRes.ok) {
            ...
            for (const imp of impulses) {
                ...
                if (!Number.isFinite(alpha) || !Number.isFinite(beta) || !Number.isFinite(budget) || !Number.isFinite(staleness))
                    continue;
            ...
            if (actionableCount > 0) {
                console.error(`[boredom-vessel] rhythm read: ${actionableCount} actionable, top=${topFamily}(${topDue.toFixed(2)})`);
            }
        }
    }
 ...
 * are proposals, etc.
 */
function goalCanFire(goalIdx, state) {
    switch (goalIdx) {
...
function recordOutcome(goalIdx, success, evidence) {
    ...
    if (success && evidence) {
        ...
        if (evidence.body && typeof evidence.body === "object") {
            const keys = Object.keys(evidence.body);
            hasMeaningfulBody = keys.length > 0 && keys.some(k => {
                const v = evidence.body[k];
...
async function fetchShapeDrivenCandidates() {
    ...
    try {
        ...
        const body = { templates: pages };
        const entries = [];
        for (const t of body.templates ?? []) {
            const tags = Array.isArray(t.tags) ? t.tags : [];
            // Match both literal and tag-prefix-normalized forms.
            const isBoredomTarget = tags.includes("boredom_target_template") ||
                tags.includes("boredomtargettemplate");
            if (!isBoredomTarget)
                continue;
            if (t.deprecated === true || t.retired === true)
                continue;
            ...
            entries.push({
                template_id: id,
                input_shapes: Array.isArray(t.input_shapes) ? t.input_shapes : [],
                output_shapes: Array.isArray(t.output_shapes) ? t.output_shapes : [],
                tags,
            });
        }
        try {
            const gapGoals = await generateGapGoalCandidates(ACTIVITY_API_ENDPOINT, API_KEY);
            for (const g of gapGoals)
                entries.push({ template_id: g.templateId, input_shapes: [], output_shapes: g.shapes, tags: [], goal_text: g.goalText });
        }
        catch {
            /* fail open */
        }
        if (templatesUnavailable && entries.length === 0)
            return candidateCache?.entries ?? [];
...
let recentShapesCache = null;
const RECENT_SHAPES_TTL_MS = 30_000;
async function fetchRecentlyProducedShapes() {
    if (recentShapesCache && Date.now() - recentShapesCache.fetchedAt < RECENT_SHAPES_TTL_MS) {
        return recentShapesCache.shapes;
    }
    const shapes = new Set();
    try {
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/execution-traces?limit=50`, { headers: { Authorization: `ApiKey ${API_KEY}` }, signal: AbortSignal.timeout(3_000) });
        if (res.ok) {
            const body = await res.json();
...
const momentumByTemplate = new Map();
...
function persistMomentum() {
    const now = Date.now();
    if (now - lastMomentumWriteAt < 5000)
        return;
...
const consecutiveIdleByTemplate = new Map();
...
const consecutiveFailureByTemplate = new Map();
...
function pruneStaleOutcomes(m) {
    const cutoff = Date.now() - OUTCOME_TTL_MS;
    while (m.outcomes.length > 0 && m.outcomes[0].at < cutoff)
        m.outcomes.shift();
}
...
const findingHistoryByTemplate = new Map();
...
const noveltyStatsByTemplate = new Map();
function recordNovelty(templateId, kind) {
    let s = noveltyStatsByTemplate.get(templateId);
    if (!s) {
        s = [];
        noveltyStatsByTemplate.set(templateId, s);
    }
    s.push({ at: Date.now(), kind });
    const cutoff = Date.now() - OUTCOME_TTL_MS;
    while (s.length > 0 && s[0].at < cutoff)
        s.shift();
    while (s.length > 50)
        s.shift();
}
function gradeNovelty(templateId, hashes) {
    if (!Array.isArray(hashes) || hashes.length === 0)
        return "novel"; // no fingerprint → assume novel (no regression)
    let hist = findingHistoryByTemplate.get(templateId);
    if (!hist) {
        hist = [];
        findingHistoryByTemplate.set(templateId, hist);
    }
    ...
    for (const h of hashes)
        hist.push(h);
    while (hist.length > NOVELTY_WINDOW)
        hist.shift();
    return fresh.length > 0 ? "novel" : "redundant";
}
function recordOutcomeByTemplate(templateId, outcome) {
    ...
    if (!m || !Array.isArray(m.outcomes)) {
        ...
        momentumByTemplate.set(templateId, fresh);
        m = fresh;
    }
    ...
    while (m.outcomes.length > 50)
        m.outcomes.shift();
    totalPicksV24f += 1;
    persistMomentum();
}
let promptSelectionPass = () => { };
function subscribeSelectionEvents(runSelectionPass) {
    let ws = null;
    let backoffMs = 1000;
        ...
        try {
            ...
            ws.onmessage = (ev) => {
                try {
                    ...
                    if (msg.type === "task.completed" || msg.type === "impulse.resolved")
                        onEvent();
                }
    ...
    try {
        ...
        for (const [tid, outcomes] of Object.entries(saved.templates ?? {})) {
            if (!Array.isArray(outcomes))
                continue;
            const m = {
                outcomes: outcomes.filter((o) => o && typeof o.at === "number" && typeof o.reward === "number"),
            };
            pruneStaleOutcomes(m);
            ...
            const recentDeltaCompletions = (dispatchLog[tid] ?? []).filter((entry) => entry.completedAt >= lookbackCutoff && entry.hasBehavioralDelta);
        ...
        for (const [tid, n] of Object.entries(saved.idle ?? {})) {
            if (typeof n === "number" && n > 0)
                consecutiveIdleByTemplate.set(tid, n);
        }
        for (const [tid, n] of Object.entries(saved.failures ?? {})) {
            if (typeof n === "number" && n > 0)
                consecutiveFailureByTemplate.set(tid, n);
        }
        console.error(`[pool] momentum hydrated: ${momentumByTemplate.size} templates, totalPicks=${totalPicksV24f}`);
    }
    catch {
...
const costResidualsMs = [];
const costResidualsTok = [];
function pushResidual(arr, actual, expected) {
    if (expected > 0) {
...
function recordCostByTemplate(templateId, ms, tokens) {
    ...
    if (prior && prior.samples.length > 0) {
        ...
        if (live.length > 0) {
            ...
            if (tok > 0 && tokLive.length > 0)
                pushResidual(costResidualsTok, tok, tokLive.reduce((s, o) => s + o.tokens, 0) / tokLive.length);
        }
    }
    ...
    while (c.samples.length > 0 && c.samples[0].at < cutoff)
        c.samples.shift();
...
let _poolMedianCache = null;
function computePoolMedians() {
    ...
    if (_poolMedianCache && Date.now() - _poolMedianCache.at < 1000)
        return { ms: _poolMedianCache.ms, tokens: _poolMedianCache.tokens };
    const cutoff = Date.now() - COST_TTL_MS;
    const msMeans = [];
    const tokMeans = [];
    for (const c of costByTemplate.values()) {
        ...
        if (live.length > 0)
            msMeans.push(live.reduce((s, o) => s + o.ms, 0) / live.length);
        const tokLive = live.filter((s) => s.tokens > 0);
        if (tokLive.length > 0)
            tokMeans.push(tokLive.reduce((s, o) => s + o.tokens, 0) / tokLive.length);
    }
    const median = (a, d) => { if (a.length === 0)
        return d; a.sort((x, y) => x - y); return a[Math.floor(a.length / 2)]; };
...
function poolMedianCostMs() { return computePoolMedians().ms; }
function poolMedianCostTokens() { return computePoolMedians().tokens; }
 ...
 */
function expectedCostMs(templateId) {
    const c = costByTemplate.get(templateId);
    if (!c || c.n < 2)
        return Infinity; // cold-start — preserve existing behaviour
...
function expectedCostTokens(templateId) {
    const c = costByTemplate.get(templateId);
    if (!c)
        return 0;
...
function combinedCostAdj(templateId) {
    const med = computePoolMedians();
    const ratios = [];
    if (med.ms > 0)
        ratios.push(expectedCostMs(templateId) / med.ms);
    if (med.tokens > 0) {
...
function writeSelectorStateSnapshot() {
    try {
        const perTemplate = [];
        ...
        const snapshot = {
            ...
            ...(() => {
                const liveOf = (arr) => arr.filter((r) => r.at >= Date.now() - COST_TTL_MS);
                const meanOf = (arr) => { const l = liveOf(arr); return l.length ? Math.round((l.reduce((s, r) => s + r.rel, 0) / l.length) * 1000) / 1000 : null; };
                const med = computePoolMedians();
                const msR = meanOf(costResidualsMs), tokR = meanOf(costResidualsTok);
                const verdict = (mr, n) => mr === null || n < 5 ? "cold" : mr <= 0.5 ? "calibrated" : "surprising";
                ...
                const vps = active.filter((t) => t.value_per_sec !== null);
                const allocEff = (() => {
                    if (vps.length < 3)
                        return null;
...
function templateMomentum(templateId) {
    const m = momentumByTemplate.get(templateId);
    if (!m || m.outcomes.length === 0)
        return 1.5; // cold-start bonus (mirrors V22)
    pruneStaleOutcomes(m);
...
function ucbScore(templateIdOrExploitation, shapeAvailOrNTotal, priorityWeightOrNChosen, unpickedCount = 1) {
...
function ucbScoreImpl(templateId, shapeAvail, priorityWeight = 1.0, unpickedCount = 1) {
    const pw = Number.isFinite(priorityWeight) && priorityWeight > 0 ? priorityWeight : 1.0;
    const m = momentumByTemplate.get(templateId);
    if (m)
        pruneStaleOutcomes(m); // V27: ensure stale outcomes don't poison the score
...
async function pickByShapeAvailability(inFlightTemplateIds) {
...
async function fetchTemplateRequiredUnboundVariables(templateId) {
    try {
...
async function fetchTemplateRequiredUnboundVariablesActivity(templateId) {
    try {
        const res = await fetch(`${ACTIVITY_API_ENDPOINT}/v2/activities/templates/${encodeURIComponent(templateId)}`, { headers: authHeaders(), signal: AbortSignal.timeout(5_000) });
        if (!res.ok)
            return null;
        const body = await res.json();
...
const gapGoalLastDispatchAt = new Map();
async function dispatchByTemplateId(templateId) {
    ...
    if (templateId.startsWith("gap-goal:")) {
        ...
        if (cand?.goal_text) {
            ...
            if (Date.now() - lastAt < GAP_GOAL_COOLDOWN_MS)
                return null;
            try {
    ...
    if (unboundVars !== null && unboundVars.length > 0) {
        console.warn(`[boredom] dispatchByTemplateId: skipping ${templateId} — required variables with no defaults: ${unboundVars.join(", ")}`);
        return null;
    }
    try {
        const res = await fetch(`${LIGHT_DISPATCH_ENDPOINT}/dispatch`, {
        ...
        if (!completed)
            reward = 0;
        else if (body.information_yield === "error")
            reward = 0; // error completion is zero-work, not productive — the curl penalty (D, 2026-06-14): stop rewarding error-circulation so high-cyclic templates decay in UCB selection
        else if (body.information_yield === "idle") {
            reward = IDLE_REWARD;
            recordNovelty(templateId, "idle");
        }
        else {
            ...
            const allArtifactsProposalReport = Array.isArray(body.finding_hashes) && body.finding_hashes.length === 0 &&
                typeof body["artifacts"] !== "undefined" &&
                Array.isArray(body["artifacts"]) &&
                body["artifacts"].length > 0 &&
                body["artifacts"].every((a) => a.kind === "proposal_report");
        ...
        console.warn(`[pool/shape] dispatch failed for ${templateId}: ${err.message}`);
...
function momentumScore(goalIdx) {
...
function statePressure(goalIdx, state) {
    const backlog = goalIdx === 8 || goalIdx === 10 ? state.openGapCount :
    ...
    if (m && m.outcomes.length >= 5) {
        ...
        if (succRate < 0.3)
            return rawPress * Math.max(0.2, succRate);
    }
    return rawPress;
}
/**
 ...
 * filter.
 */
function pickBestEligible(eligibleIdxs, state) {
    ...
    if (top.sigBoost > 1.0) {
        ...
        console.log(`[pool] signature-continuity: boosted goal[${top.idx}] by ${(top.sigBoost - 1).toFixed(2)} ` +
            `based on prior wins at signature=${sigShort}`);
        console.log(`[pool] mode=thompson cells_examined=${cellsExamined} picked=goal[${top.idx}] at signature=${sigShort}`);
    }
    return {
        idx: top.idx,
        score: top.score,
...
function inFlightHasGoal(goalIdx) {
    for (const entry of inFlight.values()) {
        if (entry.goal_idx === goalIdx)
            return true;
    }
    return false;
}
function reapStaleInFlight() {
    const now = Date.now();
    const stale = [];
    for (const [k, v] of inFlight.entries()) {
        if (now - v.started_at > IN_FLIGHT_TIMEOUT_MS)
            stale.push(k);
    }
    for (const k of stale) {
        const e = inFlight.get(k);
        inFlight.delete(k);
        console.warn(`[pool] reaped stale in-flight: dispatch_id=${k.slice(0, 8)} goal[${e.goal_idx}] ` +
 ...
 */
const recentWsCompletions = new Map();
async function reachAwareRecordOutcome(dispatchId, goalIdx, rawSuccess) { let success = rawSuccess; if (rawSuccess) {
    try {
...
} recordOutcome(goalIdx, success); } // gap momentum-reads-dispatch-status-not-reach
async function resolveExecutionIdForDispatch(dispatchId) {
    const deadline = Date.now() + 15_000;
    while (Date.now() < deadline) {
        await new Promise((r) => setTimeout(r, 1000));
        if (!inFlight.has(dispatchId))
            return; // already completed/reaped
        let res;
        try {
        ...
        if (body.executionId) {
            ...
            if (entry && !entry.execution_id) {
                ...
                console.log(`[pool] resolved execution_id for dispatch_id=${dispatchId.slice(0, 8)} ` +
                ...
                if (pending) {
                    ...
                    console.log(`[pool] completion (replay): goal[${entry.goal_idx}] (${entry.template_id ?? "?"}) ` +
        ...
        if (body.status === "completed" || body.status === "failed") {
            ...
            if (entry) {
                ...
                console.log(`[pool] early-terminal: goal[${entry.goal_idx}] (${entry.template_id ?? "?"}) ` +
 ...
 */
async function startWSObserver() {
    ...
    const connect = () => {
        ...
        try {
            ...
            console.warn(`[pool-ws] connect threw: ${err.message}`);
            ...
            try {
                ...
                for (const [dispatchId, entry] of inFlight.entries()) {
                    if (entry.execution_id === execId) {
                        ...
                        console.log(`[pool] completion: goal[${entry.goal_idx}] (${entry.template_id ?? "?"}) ` +
                ...
                if (!matched) {
                    ...
                    if (recentWsCompletions.size > 200) {
                        ...
                        if (oldest !== undefined)
                            recentWsCompletions.delete(oldest);
                    }
                }
            }
            catch (err) {
                console.warn(`[pool-ws] message parse error: ${err.message}`);
            }
        };
        ws.onclose = () => {
 ...
 */
async function dispatchOne(goalIdx, state) {
    ...
    try {
        ...
        console.warn(`[pool] dispatch failed: ${err.message}`);
        return null;
    }
    if (!res.ok && res.status !== 202 && res.status !== 207) {
        const text = await res.text().catch(() => "(no body)");
    ...
    if (decision.dispatcher === "light-dispatch") {
        ...
        const shapes = dispatch["output_shapes"];
        ...
        const inputHash = dispatch["inputStateHash"] ?? null;
        const outputHash = dispatch["outputStateHash"] ?? null;
        const isSelfReferentialOnly = dispatch["artifacts"] != null &&
            Array.isArray(dispatch["artifacts"]) &&
            dispatch["artifacts"].length > 0 &&
            dispatch["artifacts"].every((a) => a.kind === "proposal_report");
        ...
        console.log(`[pool] light-dispatch sync goal[${goalIdx}] (${targetTemplateId ?? "?"}) status=${dispatch.status} ` +
...
async function poolLoop() {
    ...
    console.log(`[pool] daemon starting: MAX_CONCURRENT=${MAX_CONCURRENT} ` +
    ...
    while (running) {
        ...
        while (inFlight.size < MAX_CONCURRENT && Date.now() - lastDispatchAt >= MIN_DISPATCH_INTERVAL_MS) {
            ...
            // candidate set.
            const inFlightTemplateIds = new Set();
            for (const e of inFlight.values()) {
                if (e.template_id)
                    inFlightTemplateIds.add(e.template_id);
            }
            ...
            if (shapePick) {
                ...
                console.log(`[pool/shape] reserving ${shapePick.template_id} score=${shapePick.score.toFixed(2)} ` +
                    ...
                    try {
                        ...
                        if (result) {
                            ...
                            if (result.success) {
                                ...
                                console.log(`[pool/shape] completed ${shapePick.template_id} ` +
                        ...
                        inFlight.delete(reserveId);
                        console.warn(`[pool/shape] dispatch error: ${err.message}`);
                    }
                })();
            ...
            for (let i = 0; i < AUTONOMOUS_GOALS.length; i++) {
                if (!goalCanFire(i, state))
                    continue;
                if (inFlightHasGoal(i))
                    continue;
                eligible.push(i);
            }
            if (eligible.length === 0)
            ...
            lastDispatchAt = Date.now();
            console.log(`[pool] reserving goal[${goalIdx}] (${AUTONOMOUS_GOAL_TARGET_TEMPLATES[goalIdx] ?? "?"}) ` +
            ...
            void (async () => {
                try {
                    ...
                    if (dispatched) {
                        ...
                        console.log(`[pool] dispatched goal[${goalIdx}] dispatch_id=${dispatched.dispatch_id.slice(0, 8)} ` +
                            `in_flight=${inFlight.size}/${MAX_CONCURRENT}`);
                    ...
                    console.warn(`[pool] dispatch goal[${goalIdx}] threw: ${err.message}`);
                }
            })();

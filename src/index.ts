...
async function writeBoredomSelectionSnapshot(input: { candidates?: unknown[]; selected?: unknown[] }): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) console.warn("[boredom] writeBoredomSelectionSnapshot: write failed", res.status, res.statusText);
  } catch (err) {
...
const AUTONOMOUS_GOALS: readonly string[] = ["development-vessel:concept-relevance-backfill-v2",
...
async function fetchShapeDrivenCandidates(): Promise<ShapeDrivenCandidate[]> {
  ...
  if (templatesUnavailable && entries.length === 0) return candidateCache?.entries ?? [];
    candidateCache = { fetchedAt: Date.now(), entries }; return entries;
  } catch {
    return candidateCache?.entries ?? [];
  }
}

...
import { VESSEL_SCAFFOLD_DISPATCH_RESULT_SHAPE, } from "../shapes/vesselScaffoldDispatchResult";
const SCENARIOS_DIR = "/workspace/validation/failure-modes/vessel-scenarios";
const RUN_GOAL_URL = "http://127.0.0.1:8210/run-goal";
const SCAFFOLD_TEMPLATE_ID = "development-vessel:scaffold-and-publish-vessel";
...
export async function resolveVesselAdditionScaffoldDispatch(ctx) {
    ...
    if (firstFile === undefined) {
        throw new Error(`No .json scenario files found in ${SCENARIOS_DIR}`);
    }
    ...
    const design = JSON.parse(llmResponse);
    ...
    if (!pr_body.includes("Substrate-Authored-By:")) {
        throw new Error("LLM-generated pr_body missing required 'Substrate-Authored-By:' trailer");
    }
    ...
    const declined = await (async () => {
        try {
            ...
            const res = await fetch(`${base}/v2/activities/templates/${encodeURIComponent(SCAFFOLD_TEMPLATE_ID)}`, {
                headers: {
    ...
    if (declined) {
        console.warn(`[vesselAdditionScaffoldDispatch] DECLINED dispatch of ${declined.template_id}: ` +
...
//# sourceMappingURL=vesselAdditionScaffoldDispatch.js.map
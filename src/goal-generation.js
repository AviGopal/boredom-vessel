 ...
 */
export function sanitizeGoalText(raw) {
...
export function admitActionableGapsMirror(gaps) {
    return gaps.filter((g) => {
        const id = String(g.id ?? "");
        const cat = String(g.category ?? "");
        const meta = (g.classification_metadata ?? g.metadata ?? {});
        ...
        if (failedAttempts >= MIRROR_EXCLUDE_ORPHAN_AFTER_FAILS)
            return false;
        // mirror: orphan_missing_shape — its route needs meta.shape
        if (cat === "orphaned_capability" && !String(meta.shape ?? "").trim())
            return false;
        return true;
    });
}
export async function generateGapGoalCandidates(activityApiEndpoint, apiKey) {
    try {
        ...
        const rawGaps = (json.body?.gaps ?? json.gaps ?? []);
        ...
        const CATEGORY_WEIGHT = { missing_capability: 3, unreachable_producer: 2.5, operational_health: 2.5, detector_coverage_gap: 2, decision_without_action: 2, posterior_consistency_drift: 1.5, architectural_pattern: 1.5, residual_shape_proposal: 1, orphaned_capability: 0.5 };
        gaps.sort((a, b) => { const wa = CATEGORY_WEIGHT[a.category ?? ""] ?? 1; const wb = CATEGORY_WEIGHT[b.category ?? ""] ?? 1; if (wb !== wa)
            return wb - wa; const da = Number(a.detected_at ? Date.parse(a.detected_at) : 0); const db = Number(b.detected_at ? Date.parse(b.detected_at) : 0); if (db !== da)
            return db - da; return String(b.detected_at ?? "").localeCompare(String(a.detected_at ?? "")); });
        ...
        try {
            ...
            if (dispatchRes.ok) {
                ...
                activeGoals = new Set(dispatches.map((d) => d.goal).filter((g) => typeof g === "string"));
            }
        }
        catch {
        ...
        for (const g of gaps) {
            ...
            if (g.id.startsWith("baseline-typecheck-broken-")) {
                ...
                if (!detectedMs || Date.now() - detectedMs > 6 * 60 * 60 * 1000)
                    continue;
            ...
            if (g.gap_subtype === "gap_backlog_unhealthy")
                continue;
            if (g.id.startsWith("auto_draft_decision"))
                continue;
            ...
            if (Array.from(activeGoals).some((goal) => goal.startsWith(`Close substrate gap ${g.id}`)))
                continue;
            if (/^\s*Close substrate gap [-\w:.!]+:?\s*$/.test(g.summary))
                continue;
            ...
            const gMeta = (g.classification_metadata ?? {});
        ...
        try {
            ...
            if (cr.ok) {
                ...
                for (const [cls] of top) {
                    ...
                    if (Array.from(activeGoals).some((goal) => goal.includes(`"${cls}" goal-failure class`)))
                        continue;
                    out.push({
                        templateId,
                        goalText,
        ...
        console.warn(`[gap-goal-supply] gap-goal candidate generation threw — 0 candidates this pass: ${e?.message ?? e}`);
        return [];
    }
}
//# sourceMappingURL=goal-generation.js.map